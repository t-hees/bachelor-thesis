...
import sturdopt.TestUtil.{dropsFiles, timedTest, wasmbenchFiles}
import sturdopt.{Parsing, Serializing, TestUtil}

import java.nio.file.{Files, Paths, StandardOpenOption}
import scala.concurrent.duration.*

class DropsTests extends org.scalatest.funsuite.AnyFunSuite {
  test("Manual test assertions") {
    dropsFiles.zipWithIndex.foreach { (p, idx) =>
      println(s"${idx}/${wasmbenchFiles.size-1}: $p")
      val result = DropsOptimization.eliminateDrops(Parsing.fromText(p))
    ...
    wasmbenchFiles.zipWithIndex.foreach { (p, idx) =>
      ...
      val testFun: () => Unit = () => {
        try {
          ...
          val result = DropsOptimization.eliminateDrops(mod)
          //prettyPrintModule(result)
